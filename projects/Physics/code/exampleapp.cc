...
namespace Example
{
	...
	public:
		...
		vec3 RasterizeLight(const float& intensity, const vec3& pixelNormal);
	...
	SoftwareRenderer::SoftwareRenderer(unsigned razWidth, unsigned razHeight) : widthImg(razWidth), heightImg(razHeight)
	{
		...
		Camera cam(90, (float)widthImg / heightImg, 0.01f, 100.0f);
	...
	inline bool SoftwareRenderer::LoadObj(const char* pathToFile, float* _coordinates, unsigned* _indices, float* _texels, float* _normals)
	{
		...
		for (size_t i = 0; i < coords.size(); i++)
		{
			...
			std::string s = (std::string("textures/res") + std::to_string(i) + std::string(".jpg")).c_str();
			std::cout << s << std::endl;
			const char* path = s.c_str();
			int res = stbi_write_png(path, widthImg, heightImg, 3, frameBuffer, widthImg * 3);
		}
	...
	vec3 SoftwareRenderer::RasterizeLight(const float& intensity, const vec3& pixelNormal)
	{
		/*
