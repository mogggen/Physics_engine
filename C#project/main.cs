...
namespace Example
{
	...
	class SoftwareRenderer
	{
		...
		vec3 defaultFragmentShader(string texture, vec3 position, vec2 uv, vec3 normal); // interpolate normals per face

		std::function<void(Vertex&)> vertexShader;
		std::function<vec3(string, vec3, vec2, vec3)> fragmentShader;
		void SoftwareRenderer::setModel_view_projection(const mat4 mvp);

		void setVertexShader(std::function<void(Vertex&)>&vertShader);
		void setFragmentShader(std::function<vec3(string, vec3, vec2, vec3)>& fragShader);
		...
		string getFrameBuffer();
		void setFrameBuffer(unsigned w, unsigned h);
		void setFrameBuffer(string frame);
		...
		vec3 getPixelColorFromUV(const string texture, const int& w, const int& h, vec2 texel);
	...
	void SoftwareRenderer::Init()
	{
		...
		std::function<vec3(string, vec3, vec2, vec3)> InitFragShader = [this](string texture, vec3 pos, vec2 uv, vec3 norm) { return defaultFragmentShader(texture, pos, uv, norm); };
	...
	void SoftwareRenderer::setFrameBuffer(string _frameBuffer)
	{
		if (fb.colorBuffer) delete[] fb.colorBuffer;
		fb.colorBuffer = _frameBuffer;
	}	

	void SoftwareRenderer::setTextureResource(string pathToResource)
	{
	...
	void SoftwareRenderer::setFragmentShader(std::function<vec3(string, vec3, vec2, vec3)>& fragShader)
	{
	...
	string SoftwareRenderer::getTextureBuffer()
	{
		return texture;
	}
	...
	string SoftwareRenderer::getFrameBuffer()
	{
	...
	vec3 SoftwareRenderer::defaultFragmentShader(string texture, vec3 point, vec2 uv, vec3 normal)
	{
	...
	vec3 printFragShader(string texture, vec3 pos, vec2 uv, vec3 normal)
	{
	...
	ExampleApp::Run()
	{
		...
		std::function<vec3(string, vec3, vec2, vec3)> frag_case_0 = [this](string texture, vec3 pos, vec2 uv, vec3 normal) { return softwareRenderer->defaultFragmentShader(texture, pos, uv, normal); };
		std::function<vec3(string, vec3, vec2, vec3)> frag_case_1 = [this](string texture, vec3 pos, vec2 uv, vec3 normal) { return printFragShader(texture, pos, uv, normal); };
		//std::function<vec3(string, vec3, vec2, vec3)> frag_case_2 = [this](string texture, vec3 pos, vec2 uv, vec3 normal) { return printFragShader(texture, pos, uv, normal); };
